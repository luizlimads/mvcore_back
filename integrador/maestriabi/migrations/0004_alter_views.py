...
CREATE OR REPLACE VIEW public.vw_maestriabi_produtos AS
 SELECT row_number() OVER (ORDER BY p.data_criacao) AS id,
    ((COALESCE(p.referencia, ''::character varying)::text || COALESCE(p.id_origem, ''::character varying)::text) || COALESCE(e.cor, p.cor, ''::character varying)::text) || COALESCE(e.tamanho, p.tamanho, ''::character varying)::text AS cod_produto,
    p.descricao AS desc_produto,
    p.grupo AS categoria,
    f.nome_fantasia AS fornecedor,
    p.marca AS nivel_1,
    COALESCE(e.cor, 'Sem Informação'::character varying) AS nivel_2,
