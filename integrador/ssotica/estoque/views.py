...
class EstoqueView(APIView):
    ...
    def post(self, request):
        importador = EstoqueImporter(
            tenant=request.user.tenant.id,
            token=request.user.tenant.api_token,
            empresa=request.user.tenant.api_user,
            loja=Loja.objects.filter(tenant=request.user.tenant.id).first().id
        )

        try:
            importador.executar()
            return Response({"mensagem": "Importação Estoque concluída com sucesso."})
